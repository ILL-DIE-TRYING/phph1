...
if(isset($valid_oneaddr) && $valid_oneaddr == 1){
	...
	*/
	if($phph1->val_getStakingTransactionsHistory($phph1->oneaddr,$pagenum,$pagesize,$fulltx,$txtype,$order)){
		$validinput = 1;
		$mytransactioncount = $phph1->hmyv2_getStakingTransactionsCount($phph1->oneaddr,$txtype);
		$trcount = $mytransactioncount['result'];
		$trpages = ceil($trcount / $pagesize);
		$hmyv2_data = $phph1->hmyv2_getStakingTransactionsHistory($phph1->oneaddr,$pagenum,$pagesize,$fulltx,$txtype,$order);
	}else{
		$validinput = 0;
	}
